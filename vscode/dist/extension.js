    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case "onGetToken": {
                    this._view?.webview.postMessage({ "type": "token", "value": TokenManager_1.TokenManager.getToken() });
                    break;
                }
                case "onLogin": {
                    if (!data.value) {
                    ...
                    await TokenManager_1.TokenManager.setToken(data.value);
                    this._view?.webview.postMessage({ "type": "token", "value": TokenManager_1.TokenManager.getToken() });
                ...
                case "onLogout": {
                    ...
                    await TokenManager_1.TokenManager.removeToken();
                    this._view?.webview.postMessage({ "type": "token", "value": TokenManager_1.TokenManager.getToken() });
...
function activate(context) {
    ...
    const insert = vscode.commands.registerCommand('autologger.insert', async ({ position, content, selection }) => {
        const editor = vscode.window.activeTextEditor;
        if (editor == null) {
            return;
        }
